...
class SentController extends Controller
{
    ...
    public function index($get_gid= null)
    {
        if($get_gid){
             $gid = $get_gid;
              $navi_menu = 'ผอ.ลงนามแล้ว(แทน)';
        }else{
            $gid = session()->get('gid');
            $gid=$gid['gid'];
             $navi_menu = ($gid == 10? 'ผอ.ลงนามแล้ว' : 'ส่งเอกสาร' ) ;
        }
//        $navi_menu = 'ส่งเอกสาร';
...
//                 dd($department_id);
		...
		$item->DETAIL = $request->input('DETAIL');
                ...
                $item->CREATE_USER =  $userdata['username'];
		$item->save();
                }
//                //ส่วนงานทั่วไป
                else{
...
//           dd($userdata);
           ...
           date_default_timezone_set("Asia/Bangkok");
            $result = true;     
//            print_r(count($document_item_id));
...
//                dd($item_id);
		...
		$item->save();
              ...
              public function director_control_code(Request $request)
    {
          $userdata = session()->get('userdata');
